...
import { getEmail, clearAuthCookies, isAdmin, isSystem } from '@/lib/auth'
...
export default function WebmailApp() {
  ...
  const adminLink = isSystem() ? '/system' : isAdmin() ? '/admin' : null
  const adminLabel = isSystem() ? 'System' : 'Admin'
      ...
      <header className="h-14 flex-shrink-0 flex items-center justify-between px-4 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
        <div className="flex items-center gap-2.5">
          ...
          {adminLink && (
            <Link
              href={adminLink}
              className={`ml-1 px-2 py-0.5 text-[11px] font-semibold rounded ${
                isSystem()
